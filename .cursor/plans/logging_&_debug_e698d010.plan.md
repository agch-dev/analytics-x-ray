...
**Option A: Keep emojis (dev only)**
Since we're disabling logs in production anyway, emojis only affect developers.

**Option B: Replace with prefixes**
```typescript
...
**Option C: Use log level indicators only**
Let the logger prefix handle it, remove inline emojis:
```typescript
log.info('Background service worker loaded');
...
| File | Changes |
|------|---------|
| `src/lib/logger.ts` | Add `__DEV_MODE__` check to default config |
| `src/lib/storage.ts` | Gate `logStorageSize` output behind dev mode |
...
2. Run production build - console should be quiet (only errors/warnings)
3. Verify no runtime errors from logger in production
