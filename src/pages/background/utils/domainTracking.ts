...
export function updateTabDomainInfo(tabId: number, url: string): void {
  ...
  log.debug(`🌐 Updated domain info for tab ${tabId}: ${domain} (normalized: ${normalizedDomain}, allowed: ${isAllowed})`);
  
  // Log detailed matching info for debugging
  if (!isAllowed) {
    log.debug(`🔍 Domain ${domain} not in allowlist. Allowed domains:`, domainStore.allowedDomains.map(d => `${d.domain} (subdomains: ${d.allowSubdomains})`));
  }
